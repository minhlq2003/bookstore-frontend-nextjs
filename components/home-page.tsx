      ...
      try {
        ...
        if (response && Array.isArray(response.data) && response.data.length > 0) {
          const transformedBooks: Book[] = response.data.map((apiBook: any) => ({
            ...
            description: String(apiBook.description || "No description available."),
            price: parseFloat(apiBook.price) || 0,
            book_images: Array.isArray(apiBook.book_images) && apiBook.book_images.length > 0
              ? apiBook.book_images.map((img: any) => ({ id: img.id, url: String(img.url) }))
              : [ { id: 0, url: Images.bookImg.src || "/default-image.jpg" } ],
            sold: Number(apiBook.sold || 0)
          }));
          setBestSellingBooks(transformedBooks);
          setFeaturedBook(transformedBooks[ 0 ]);
        } else {
          setErrorHero("No best selling books found or data is not in the correct format.");
          setBestSellingBooks([]);
          setFeaturedBook(null);
        }
      } catch (err) {
        console.error("Error while fetching best selling books: ", err);
        setErrorHero("Unable to load best selling books. Please try again later!");
        setBestSellingBooks([]);
        setFeaturedBook(null);
      } finally {
      ...
      <div className="w-full bg-[#ececec] sm:pt-5 pt-0 pb-20">
        ...
        <div className="max-w-[1440px] mx-auto bg-gray-200 rounded-xl overflow-hidden">
          <div className="bg-[#0B3D9180] text-white py-20 sm:py-32 px-7">
            <div className="max-w-[1440px] mx-auto text-center">
              ...
              <p className="text-lg sm:text-xl mb-8">
                Discover a creative world through books!
                We offer a vast collection to suit every taste, along with special offers just for you.
              </p>
            </div>
          </div>
          ...
          <div className="mx-auto sm:p-6 p-3 bg-[#0B3D91] text-white">
            { loadingHero && (
              <div className="text-center py-10">Loading best selling books...</div>
            ) }
            { errorHero && (
                  ...
                  { featuredBook ? (
                    <>
                      ...
                      <div className="flex sm:space-x-4 space-x-2 items-center">
                        <button
                          onClick={ () => handleViewDetailsClick(featuredBook.id) }
                  ...
                  { bestSellingBooks.map((book, index) => (
                    <div
                      ...
                      style={ {
                        ...
                        left: `${index * 15 + 5}%`,
                        transform: featuredBook?.id === book.id ? 'scale(1.1) translateY(-10px)' : 'scale(1)',
                        filter: featuredBook?.id === book.id ? 'drop-shadow(0 0 10px rgba(255,255,255,0.7))' : 'none',
                      } }
