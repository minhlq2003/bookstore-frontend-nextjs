...
export default function EditCategory() {
  ...
  const onFinish = async (values: CategoryFormValues) => {
    try {
      await axiosClient.put(`/categories/${categoryId}`, values)
  ...
  return (
    <div className='min-h-[85vh] w-full bg-white dark:bg-gray-900 flex flex-col items-start justify-start rounded-lg shadow-sm gap-4 px-4 pt-10'>
      <div className='w-1/2'>
        <Title level={2} className='m-0'>
          Chỉnh sửa danh mục
        </Title>
        {!loading && category ? (
          <CategoryForm form={form} onFinish={onFinish} onFinishFailed={onFinishFailed} />
        ) : (
          <p>Đang tải...</p>
        )}
      </div>
